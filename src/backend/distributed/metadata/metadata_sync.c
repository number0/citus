...
PG_FUNCTION_INFO_V1(sync_metadata_to_node);


/*
 * sync_metadata_to_node function creates the metadata in a worker for preparing the
 ...
 */
Datum
sync_metadata_to_node(PG_FUNCTION_ARGS)
{
	...
	PreventTransactionChain(true, "sync_metadata_to_node");
	...
	if (workerNode == NULL)
	{
		ereport(ERROR, (errcode(ERRCODE_OBJECT_NOT_IN_PREREQUISITE_STATE),
						...
						errhint("First, add the node with master_add_node().")));
	}
